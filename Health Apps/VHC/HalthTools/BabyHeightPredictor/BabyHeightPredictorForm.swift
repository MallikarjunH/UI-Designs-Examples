...
class BabyHeightPredictorForm: UIViewController,UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate,HeightSelectionPopUpVCDelegate {
    ...
    var isFatherHeightAlreadySelected = false
    var isMotherHeightAlreadySelected = false
 ...
 /*   func textFieldDidEndEditing(_ textField: UITextField){
        ...
        if textField.tag == 11 //father
        {
            parentsHeightArray[0] = textField.text ?? "0"
        ...
        else if textField.tag == 12 //mother
        {
            parentsHeightArray[1] = textField.text ?? "0"
    ...
    @objc func selectHeightPopUpForFather(sender:UIButton)
    {
         if isGenderButtonSelected {
           ...
           showAlert(message: "Please select the gender of your child", title: "Alert")
        }
    ...
    func SuggistionPopUpSelected(_ index: Int, _ selectedOption: String) {
        ...
        if selectHeightPopUpOptionFor == "f" {
           parentsHeightArray[0] = selectedOption //for father
            ...
            if parentsHeightArray[0] != "0" &&  parentsHeightArray[0] != ""{
                if isFatherHeightAlreadySelected {
                    
                }else{
                    progressPercentageValue = progressPercentageValue + 33.33
                    isFatherHeightAlreadySelected = true
                }
          ...
          parentsHeightArray[1] = selectedOption //for mother
            ...
            if parentsHeightArray[1] != "0" &&  parentsHeightArray[1] != ""{
                ...
                if isMotherHeightAlreadySelected {
                    
                }else{
                    progressPercentageValue = progressPercentageValue + 33.33
                    isMotherHeightAlreadySelected = true
                }
    ...
    @IBAction func calculateButtonClicked(_ sender: Any) {
        ...
        if !isGenderButtonSelected{
           ...
           // print("Calculate result ")
            ...
            GlobalVariables.sharedManager.heightArrayOfFatherAndMotherForHp = parentsHeightArray
            GlobalVariables.sharedManager.genderTypeForBFC = babyGenderValue
